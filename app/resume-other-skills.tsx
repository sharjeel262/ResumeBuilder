...
const ResumeOtherSkillsScreen = () => {
  ...
  const handleNext = async () => {
    ...
    if (error) {
      ...
      router.push({ pathname: '/resume-experience', params: { resume_id } });
    }
  };

  return (
    <ScrollView contentContainerStyle={[styles.container, { backgroundColor: colors.background }]}>
      <Text style={[styles.title, { color: colors.text }]}>Other Skills</Text>
      {skills.map((skill, idx) => (
        <View key={idx} style={[styles.skillBlock, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          <TextInput
            label={`Other Skill ${idx + 1}`}
            value={skill}
...
export default ResumeOtherSkillsScreen; 